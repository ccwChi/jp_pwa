...
Dev-only authoring tools need a real Node server and cannot survive static export, so the deploy workflow deletes `src/app/api` wholesale before building (plus `src/app/reading/import` and `src/app/practice/tag`, whose UIs live outside `src/app/api`): `src/app/api/articles/import/route.js` (writes a new article `./data` file to disk from the import wizard) and `src/app/reading/import/` (the wizard UI itself, `useImportWizard.js` + `ImportTool.js`); `src/app/api/practice-tag/route.js` + `src/app/practice/tag/` (the POS-tagging tool); and `src/app/api/practice-explain/route.js` (the "編輯生字與解析" panel inline in `src/app/practice/exam/[examId]/ExamPracticeClient.js`, gated client-side by `NEXT_PUBLIC_STATIC_EXPORT` so the edit UI itself never renders on the deployed site). All three write routes share `src/lib/practice/bank/devTagStore.js`, which reads/writes `./data/*.js` files directly on disk. `npm run build:pages` reproduces the exclusion locally by temporarily moving `reading/import` and `practice/tag` (and all of `src/app/api`) out of `src/app` and back.
